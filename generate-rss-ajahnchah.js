...
const HISTORY_FILE = 'history-ajahnchah.json';
const RSS_FILE = 'ajahnchah.xml';
const CYCLE_FILE = 'cycle-state-ajahnchah.json'; // New file to track the current cycle
const MAX_ITEMS = 10;

// 1. Read the JS file containing the Ajahn Chah quotes
const fileContent = fs.readFileSync('ajahnchah-quotes.js', 'utf8');
const match = fileContent.match(/const AjahnChahQuotesData = `([\s\S]*?)`;/);
if (!match) {
    console.error("Could not find the Ajahn Chah quotes data.");
    process.exit(1);
}
